...
async function check() {
  ...
  snap.forEach(doc => {
    console.log(doc.id, doc.data().date, doc.data().driverName, doc.data().timestamp);
  });
  
  const snapAbast = await getDocs(query(collection(db, "abastecimentos"), orderBy("createdAt", "desc"), limit(5)));
  console.log("\nAbastecimentos:");
  snapAbast.forEach(doc => {
    console.log(doc.id, doc.data().date, doc.data().driverName, doc.data().createdAt);
  });
  process.exit(0);
}
check();
